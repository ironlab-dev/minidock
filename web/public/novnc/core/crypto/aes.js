export class AESECBCipher {
    ...
    async _importKey(key, extractable, keyUsages) {
        if (!window.crypto || !window.crypto.subtle) {
            throw new Error("Web Crypto API (window.crypto.subtle) is not available. This may be due to:\n" +
                "1. Using an insecure context (HTTP instead of HTTPS)\n" +
                "2. Browser security settings\n" +
                "3. Running in an iframe with restricted permissions\n" +
                "Please ensure you're using HTTPS or check your browser settings.");
        }
